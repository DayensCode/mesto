...
const avatarForm = document.forms["confirm"];
...
popupOpenChangeElement.addEventListener("click", () => {
	...
	avatarPopup.getInputValues(user.getUserInfo());
});
...
Promise.all([api.getInitialCards(), api.getUserInfo()]).then(([serverCards, userData]) => {
	...
	const card = new Card(userId, dataCard, "#element-template", {
		...
		handleLikeClick: (id) => {
			card.hasMyLike() //удалим если имеется наш
				...
				}).catch((err) => console.log(err)) : api.addLike(id) //добавим если нет нашего
				.then((res) => {
					card.updateCounter(res.likes);
		...
		handleDeleteClick: () => {
			...
			modalConfirm.setSubmit(() => {
				api.deleteCard(dataCard._id).then(() => {
...
const modalEdit = new PopupWithForm(".popup_edit", (data) => {
	...
	api.updateUserInfo(data.name, data.about).then(res => user.setUserInfo(res.name, res.about)).then(() => modalEdit.closePopup()).catch((err) => console.log(err)).finally(modalEdit.changeButtonText);
});
modalEdit.setEventListeners();
const modalAdd = new PopupWithForm(".popup_add", (values) => {
	modalAdd.changeButtonText("Сохранение...");
	api.addCard(values).then(res => cardList.addItem(createCard(res))).then(() => modalAdd.closePopup()).finally(modalAdd.changeButtonText);
});
...
function handleEditAvatar() {
	...
	api.updateAvatar(avatarInput.value).then(res => user.setAvatar(res.avatar)).then(() => avatarPopup.closePopup()).catch((err) => console.log(err)).finally(avatarPopup.changeButtonText)
}
